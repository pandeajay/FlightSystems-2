...
public class NeoGraph implements Graph{
	...
	public NeoGraph(){
		try{
			graphDb = new GraphDatabaseFactory().newEmbeddedDatabase( "E:\\FlightSystems\\EmbeddedDB\\" );
	...
	public long createNode(src.graphs.node.Node node) {
		...
		try ( Transaction tx = graphDb.beginTx() ){
			...
			// Label label = new DynamicLabel(null);
			// myNode.addLabel(arg0);
	...
	public void createEdge(src.graphs.node.Node node) {
		try ( Transaction tx = graphDb.beginTx() ){
			long fromId = Long.parseLong(NeoGraph.nodeIAndNeoId.get(node.id));
			Node fromNode = graphDb.getNodeById(fromId);			
			Iterator<Entry<String, String>> it = node.to.entrySet().iterator();
			while(it.hasNext()){
				Entry<String, String> entry = it.next();
				long toId = Long.parseLong(NeoGraph.nodeIAndNeoId.get(entry.getKey()));
			...
			System.out.println("Exceptio in NeoGraph createEdges " + e);
		}
